...
// Controller of guia page.
appControllers.controller('buscarLocalCtrl', function ($scope, $mdToast, $state) {
	
    $scope.openBuscar = function (busqueda) {
	   // if (settings.busqueda=='undefined') {settings.busqueda=''; }
	   
         $scope.isAnimatedx=false;
	   
	      //if (settings.busqueda === undefined) {   settings.busqueda ='';  }
		 // if (settings.categoria === undefined) {   settings.categoria ='';  }
             $state.go("app.contractlist", {
				filterText: busqueda,
				isAnimated:isAnimatedx
			 });
			//});
				//window.open("https://villalaangostura.org/comercios/?action=search&dosrch=1&q&listingfields%5B1%5D="+settings.busqueda+"&listingfields%5B2%5D="+settings.categoria, '_blank', 'location=no');
...
appControllers.controller('contractListCtrl', function ($scope, $stateParams,$filter, $mdDialog, $timeout, $ionicModal, $state, $mdBottomSheet, ContractDB) {
    ...
    $scope.initialForm = function () {
       ...
       // $timeout(function () {
        ...
        //}, 30);// End loading progress.3000
		
		
        $timeout(function () {
			alert("deberia carga datos");
            $scope.getContractList();
			jQuery('#contract-list-content').show();
			$scope.isLoading = false;
...
appControllers.controller('contractDetailCtrl', function ($mdBottomSheet, $mdToast, $scope, $stateParams, $filter, $mdDialog, $ionicHistory, ContractDB, $ionicHistory) {
    ...
    $scope.saveContract = function (contract, $event) {
        ...
        $mdDialog.show({
            ...
            locals: {
                displayOption: {
                    title: "Confirma Guardar dato?",
                    content: "Se guardó el dato.",
                    ok: "Confirmar",
                    cancel: "Cerrar"
                }
            }
    ...
    $scope.deleteContract = function (contract, $event) {
        ...
        $mdDialog.show({
            ...
            locals: {
                displayOption: {
                    title: "Confirma elimanr dato?",
                    content: "Se eliminó el dato.",
                    ok: "Confirmar",
                    cancel: "Cerrar"
                }
            }
    ...
    $scope.validateRequiredField = function (form) {
        return !(   (form.contacto.$error.required == undefined)
        && (form.categoria.$error.required == undefined)
...
appControllers.controller('contractSettingCtrl', function ($scope,$ionicViewSwitcher,$state, $timeout, $stateParams, $mdDialog, $mdBottomSheet, $mdToast, $ionicHistory, ContractDB) {
    ...
    $scope.clearAllData = function ($event) {
        ...
        $mdDialog.show({
            ...
            locals: {
                displayOption: {
                    title: "Confirma eliminar todos lo datos?",
                    content: "Se eliminaron todos los datos.",
                    ok: "Confirmar",
                    cancel: "Cerrar"
                }
            }
